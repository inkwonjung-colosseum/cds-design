...
await page.waitForSelector(".planner__sessions .row", { timeout: 30000 });
...
await page.waitForSelector(".planner__sessions .row", { timeout: 20000 });
check("session list repopulates after reconnect", true);

// --- RPCs work again: delete through the UI ------------------------------
const rows = page.locator(".planner__sessions .row");
const firstRow = rows.first();
const targetId = await firstRow.getAttribute("data-session-id");
await firstRow.hover();
await firstRow.locator("..").locator(".row-delete").click();
await page.waitForFunction(
  (id) => !document.querySelector(`.planner__sessions .row[data-session-id="${id}"]`),
  targetId,
  { timeout: 15000 },
);
check("session delete works after reconnect", true, targetId ?? "unknown id");
