...
async function main() {
  ...
  try {
    ...
    // --- 기획서 게시: the mirror's own publish, over the page tree ----------
    // The button counts what would be sent; the resolved page is still
    // "modified" (its hash was cleared so the next push overwrites the
    // remote), so it is exactly one.
    const publish = page.getByRole("button", { name: /^기획서 게시( \(\d+\))?$/ });
    await publish.waitFor({ timeout: 10000 });
    check(
      "기획서 게시 counts the pages waiting to go up",
      /게시 \(1\)/.test(await publish.innerText()),
      await publish.innerText(),
    );
    ...
    // --- 이 문서로 화면 만들기: same page, a 화면 thread, the screen segment --
    await page.getByRole("button", { name: "이 문서로 화면 만들기" }).click();
    ...
    check(
      "the brief is prefilled but not sent",
      drafted.includes("@confluence/ENG/회원 관리 기획서.md") &&
